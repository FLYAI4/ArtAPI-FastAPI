...
from src.libs.api.util import delete_file
...
async def test_can_resize_image():
    await delete_file(RESIZED_IMAGE_PATH)
    # given : 유효한 이미지
    # when : 이미지 사이즈 변경 요청
...
async def test_can_get_generated_video():
    await delete_file(GENERATE_ORIGIN_VIDEO_PATH)
    # given : 생성된 비디오 요청 id
    # when : 비디오 전달 요청
...
async def test_can_reverse_generated_video():
    await delete_file(REVERSED_VIDEO_PATH)
    # given : 유효한 비디오 파일
...
@pytest.mark.order(5)
@pytest.mark.asyncio
async def test_can_slow_generated_video():
    await delete_file(SLOW_VIDEO_PATH)
    # given : 유효한 비디오 파일
    # when : 비디오 파일 역 재생 및 연결
    # 비디오 속성 가져오기
